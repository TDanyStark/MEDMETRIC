...
export function RepHistoryPage() {
  ...
  const sessionsQuery = useQuery({
    ...
    queryFn: () =>
      listRepSessions({ page, q: q || undefined, date: date || undefined }),
  });
    ...
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-8 px-4 py-8 sm:px-6 lg:px-8 animate-in fade-in duration-500">
      <div>
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          Revisa las sesiones que has creado y agrega más materiales cuando lo
          necesites.
        </p>
      </div>
      ...
      <div className="flex flex-col gap-6">
        <SearchToolbar
          ...
          onChange={(val) =>
            setSearchParams((prev) =>
              updateSearchParams(prev, { q: val || null, page: 1 }),
            )
          }
          extra={
        ...
        {!sessionsQuery.isLoading &&
          ...
          sessionsQuery.data?.items.length === 0 && (
            <EmptyState
              title="Sin sesiones"
              description="Crea tu primera visita médica desde la biblioteca."
            />
          )}

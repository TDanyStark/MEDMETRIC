...
import { getOrgMaterialPreview } from '@/services/backoffice'
import { Button } from '@/components/ui/Button'

interface OrgPreviewDialogProps {
  isOpen: boolean
  onOpenChange: (open: boolean) => void
  material: Material | null
}

export function OrgPreviewDialog({ isOpen, onOpenChange, material }: OrgPreviewDialogProps) {
  const { data: resource, isLoading, isError } = useQuery({
    queryKey: ['org-admin', 'materials', material?.id, 'preview'],
    queryFn: () => getOrgMaterialPreview(material!.id),
          ...
          {isLoading ? (
            ...
            <div className="flex h-full flex-col items-center justify-center p-10 text-center">
              ...
              <Button
                size="lg"
                onClick={() =>
                  window.open(resource.url ?? material?.external_url ?? '', '_blank')
                }
              >
